...
考虑无遮挡的直接光，其中 $\rho(x)$ 为 BRDF 函数，$G(x)$ 为几何项 ${(\cos \theta^x_i \cos \theta^{x'}_{o})}/{| x - x' |^2}$，则利用直接光采样算法，我们可以得到 $x$ 和对应的 pdf $p(x)$，且我们大概知道
$$
p(x) \propto L_e(x) G(x)
$$

但是，我们知道我们的“终极目标”是抽到符合分布 $\hat p(x) = \rho(x) L_o(x) V(x) G(x)$ 的样本。我们可以用前面使用 $p(x)$ 抽到的 $x$ 进行 WRS，再利用 RIS 的积分估计量得到最终的积分估计。

同时，时间上临近（前一帧相同或利用 motion vector 指出的相似位置）和空间上临近（相邻的像素）的 $x$ 样本，其在进行抽样时很可能使用了相似的分布 $p(x)$，所以也可以作为有效的样本加以重用。

不过，我们也会注意到几个问题：
1. 时间 / 空间上临近的样本，抽样直接光所用的分布 $p_{{other}}(x)$ 其实并不完全符合本像素的分布 $p(x)$
2. RIS 积分估计量的无偏性要求 $\forall x \in \operatorname{supp}(f),p(x) > 0$ 
